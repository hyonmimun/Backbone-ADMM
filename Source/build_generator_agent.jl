function build_generator_agent!(mod::Model, market_design::AbstractString)
    ...
    if market_design == "EOM"
        ...
        # cfd variables
        Q_cfd = mod.ext[:variables][:Q_cfd] = @variable(mod, lower_bound=0,base_name="CfD_contracted_capacity") # cfd contracted capacity [GW/year]
        ...
        cfd_generator_profit = mod.ext[:expressions][:cfd_generator_profit] = @expression(mod,[jy=JY], cfd_payout_gen[jy] + generator_profit[jy] - gen_cfd_costs[jy]) # 10^6€-year
        ...
        ) # total sold under eom and cfd = together cannot exceed available capacity
