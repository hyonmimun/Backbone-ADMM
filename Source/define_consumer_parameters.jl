function define_consumer_parameters!(mod::Model, data::Dict, ts::Dict, market_design::AbstractString, results::Dict)
    ...
    [ts[jy][!,Symbol(data["D"])][idx(jy,jd,jh)] for jh=1:nT, jd=1:nR, jy=1:nY]/1000 # demand profile for segment in GW

    D_PV = data["totConsumers"]*data["Share"]*data["PV_cap"].*
    [ts[jy][!,Symbol(data["PV_AF"])][idx(jy,jd,jh)]/1000 for jh=1:nT, jd=1:nR, jy=1:nY]
    ...
    mod.ext[:parameters][:D_ELA_max] = 0.2 .* D_consumers # Elastic demand (20%)
    ...
    if market_design == "cfd"
        ...
        mod.ext[:parameters][:g_cfd] = zeros(nT,nR,nY) # generation under cfd
        ...
        if !haskey(mod.ext[:parameters], :Q_cfd_con_tot)
            mod.ext[:parameters][:Q_cfd_con_tot] =
                ...
                copy(last(results["Q_cfd_con_tot"])) : fill(1e-8, length(JY))  # of fill(1e-8, nY)
        end
    end
end
    return mod