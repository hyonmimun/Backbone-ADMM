function ADMM_subroutine!(m::String,results::Dict,ADMM::Dict,EOM::Dict,mod::Model,agents::Dict,TO::TimerOutput,market_design:: AbstractString)
    ...
    @timeit TO_local "Compute ADMM penalty terms" begin
        ...
        mod.ext[:parameters][:Q_cfd_bar] = results["Q_cfd"][m][end] - 1/(EOM["nAgents"])*ADMM["Imbalances"]["cfd"][end]
    ...
    if m in agents[:Gen]
        ...
        #println("Q_cfd_bar for generator $m:", mod.ext[:parameters][:Q_cfd_bar])
        #=    # Add debug prints
        println("\nSolver Status for Generator $m:")
        status = optimize!(mod)
    ...
    elseif m in agents[:Cons]
        ...
        #println("Q_cfd_bar for consumer $m:", mod.ext[:parameters][:Q_cfd_bar])

        #= Add debug prints
        println("\nSolver Status for Consumer $m:")
        status = optimize!(mod)
        println("Termination Status: ", termination_status(mod))
        println("Primal Status: ", primal_status(mod))
        println("Dual Status: ", dual_status(mod))
