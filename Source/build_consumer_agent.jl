function build_consumer_agent!(mod::Model)
    ...
    # CfD parameters
    cfd_share = mod.ext[:parameters][:cfd_share] # % of total demand hedged by CfD
    ...
    SOC = mod.ext[:variables][:SOC] = @variable(mod, [jh=JH], lower_bound = 0,upper_bound = cap_smax, base_name="SOC") # state of charge
    ...
    g[jh] == -(D_fixed[jh] + D_ELA[jh] - PV[jh]) - charge[jh] + discharge[jh] # Grid exchange exactly reflects necessary generation to meet total demand accounting for pv and battery
    ) 
