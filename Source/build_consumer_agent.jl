function build_consumer_agent!(mod::Model)
    ...
    utility_term = mod.ext[:expressions][:utility_term] = @expression(mod, [jh in JH], WTP * D_ELA[jh] - (WTP/(D_ELA_max[jh]) * (D_ELA[jh]^2)))
    ...
    g[jh] <= -(D_fixed[jh] + D_ELA[jh] - PV[jh]) - charge[jh] + discharge[jh] # Battery can only charge from PV production or the grid
    )
